...
for dataset in [160]:
    for mode in ['naive']:
        if dataset==40:
            train_data=EraiCpcDataset('./tensordata-precip-40','train')
            val_data=EraiCpcDataset('./tensordata-precip-40','val')
            test_data=EraiCpcDataset('./tensordata-precip-40','test')
        else:
        ...
        train_loader=torch.utils.data.DataLoader(train_data,batch_size=4,shuffle=True,num_workers=1,pin_memory=True,sampler=None,drop_last=True)
            ...
            with torch.no_grad():
                ...
                if total_loss<loss_max:
                    ...
                    if dataset==40:
                        ...
                        torch.save(network.state_dict(),'_autoencoder_160_'+mode+'.pt')
